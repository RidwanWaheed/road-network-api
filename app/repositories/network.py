...
class NetworkRepository(BaseRepository[Network, NetworkCreate, NetworkUpdate]):
    ...
    def get_by_customer(self, db: Session, *, customer_id: int, skip: int = 0, limit: int = 100) -> List[Network]:
        return db.query(Network).filter(Network.customer_id == customer_id).offset(skip).limit(limit).all()

    def create_with_version(self, db: Session, *, obj_in: NetworkCreate, customer_id: int) -> Network:
    ...
    def get_latest_version(self, db: Session, *, network_id: int) -> Optional[NetworkVersion]:
        """Get the latest version of a network"""
        return db.query(NetworkVersion).filter(
    ...
    def create_new_version(self, db: Session, *, network_id: int) -> NetworkVersion:
        ...
        db_version = NetworkVersion(network_id=network_id, version_number=new_version_num)
        db.add(db_version)
        db.commit()
